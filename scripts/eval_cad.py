...
def eval_cls_k_click_frame_t(cls, k, t):
    ...
    video_dir = test_data_folder + cls + '/new_frames/'
    gt_dir = test_data_folder + '../new_gt/' + cls + '/groundtruth/'
    ...
    if prompt_type == 'point':
        for k in click_numbers:
            for t in prompt_frame:
                file = '../eval_result/CAD_%s_%s_%s_frame_%s.json'%(model_type, k, prompt_type, t)
                print(file)
        ...
        for t in prompt_frame:
            file = '../eval_result/CAD_%s_%s_frame_%s.json'%(model_type, prompt_type, t)
            print(file)
            with open(file, 'w') as f:
                results = {}
                for cls in classes:
